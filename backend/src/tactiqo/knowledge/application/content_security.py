...
class DocumentContentSecurityScanner:
    ...
    _PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
        ...
        (
            ...
            re.compile(
                ...
                r"|(?:رسالة|تعليمات)\s+(?:النظام|المطور|المساعد)",
                re.IGNORECASE,
            ),
        ),
    )
    ...
    def model_evidence(self, text: str) -> str | None:
        """Quarantine suspicious chunks instead of exposing them to model context."""
        if self.assess(text).suspicious:
            return None
        return self.wrap_as_evidence(text)
